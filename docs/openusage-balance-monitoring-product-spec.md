...
- 官方 OAuth 模式显示 Provider、计划名称和使用窗口。
- API 密钥模式显示 Provider 以及项目、会话标题、模型和输入/输出 Token。无可用 OAuth 凭据时，自定义 Base URL 和第三方推理配置统一归入 API 密钥模式。
- 两种主体内容互斥，不会同时出现。
- 两种模式都可以显示当前会话的上下文百分比。
- 实时使用情况不可用不得改变 Halo 的会话状态，也不得让整个服务商（Provider）进入错误状态。

详情面板不显示“使用情况 / 余额”“会话详情”、`OAuth` 或 `API Key` 等分区或模式标签。本期只实现使用情况；余额、Credits、额外使用额（Extra Usage）和速率限制重置额度（Rate Limit Reset Credits）属于后续能力，本期不获取、不展示，也不占位。
...
4. 无法确认凭据类型时，安全降级为 API 密钥模式并显示本地会话字段，不显示服务商错误。
...
- Provider 行：固定显示在 Agent 切换器下方；OAuth、API 密钥和离线状态均显示当前 Provider 名称。
- OAuth Provider 行在名称后显示计划名称；存在 Usage 异常时紧跟一个黄色感叹三角形，悬停显示脱敏错误说明。
- 当前状态：`THINKING`（思考中）、`WORKING`（工作中）、`NEEDS YOU`（需要您）、`DONE`（已完成）、`OFFLINE`（离线）或等效的本地化文案。
- 当前任务详情：当前操作。

面板不显示认证模式标签或主体分区标题。Provider 行中的名称使用主文本，计划名称使用较小的次要文本；计划为空时不显示占位。API 密钥模式只显示 Provider 名称，不显示计划或 Usage 状态图标。
...
- Provider 行中的映射后计划名称。
- `5-Hour` 与 `Weekly` 两个使用窗口；中文分别显示“五小时”和“每周”。
- 每个窗口的进度条、剩余百分比和重置时间；进度条填充长度表示剩余额度。
- 异常数据状态对应的黄色感叹三角形及 Tooltip。

OAuth 模式不显示四行本地会话字段。本期不显示货币余额、Credits、Extra Usage 或 Rate Limit Reset Credits。
...
- API 密钥模式不显示使用窗口及其不可用占位行。
- 有当前准确会话的上下文数据时仍显示 `contextPill`；该指标不属于上述四行字段。
...
- `subscriptionType` 为空时，Provider 行不显示计划名称。

## 数据状态与缓存

数据状态仅影响 OAuth Provider 行中的黄色感叹三角形及其 Tooltip，不增加底部状态行。刷新间隔由实现配置，默认 5 分钟。

状态优先级从高到低如下：

1. OAuth 凭据失效、缺少实时使用情况所需的 Scope 或 Token 刷新失败：保留当前账户最后一次成功快照（如果存在），显示黄色三角形；Tooltip 提示重新登录对应 Provider。
2. API 返回 429：保留最后成功快照（如果存在），显示黄色三角形；Tooltip 提示请求过于频繁并将在稍后重试。
3. 本次刷新失败但存在当前账户的成功快照：继续显示快照，显示黄色三角形；Tooltip 提示数据可能已过期并给出最后更新时间。
4. 本次刷新失败且当前账户从未获得成功快照：两个窗口显示“暂无数据”，显示黄色三角形；Tooltip 给出脱敏后的失败说明。
5. 最近一次成功更新超过两个刷新周期：继续显示快照，显示黄色三角形；Tooltip 提示数据可能已过期并给出最后更新时间。
6. 最近一次成功更新距今不超过两个刷新周期：正常显示，不显示三角形或更新时间文案。

首次刷新尚未完成且没有失败分类时不显示三角形；正常刷新进行中也不新增加载动画或错误图标。
...
- 同一时刻只显示一个黄色三角形和一条 Tooltip 文案，按上述优先级选取内容。
- 正常后台刷新不显示加载动画；错误图标使用黄色而非红色，也不改变 Halo 状态。
...
- Provider：显示 `Codex`；离线时仍保留。
- 计划：在 Provider 名称后按“计划名称映射”显示。
- `5-Hour`：5 小时主要窗口，显示进度条、剩余百分比和重置时间。
- `Weekly`：7 天次要窗口，显示进度条、剩余百分比和重置时间。
- 数据异常：按“数据状态与缓存”显示黄色三角形及 Tooltip。
- 不显示四行本地会话字段、货币余额、Credits、Extra Usage 或 Rate Limit Reset Credits。
...
- 无法确认 Codex 凭据类型，需要安全降级为本地会话字段。

### 面板内容

Provider 行显示 `Codex`，主体区域显示四行独立字段：项目、会话标题、模型、输入/输出 Token。

### 要求行为

- 不显示计划、5-Hour/Weekly 使用窗口、Usage 状态图标或任何使用情况占位行。
...
- Provider：显示 `Claude Code`；离线时仍保留。
- 计划：在 Provider 名称后按“计划名称映射”显示。
- `5-Hour`：5 小时使用窗口，显示进度条、剩余百分比和重置时间。
- `Weekly`：7 天使用窗口，显示进度条、剩余百分比和重置时间。
- 数据异常：按“数据状态与缓存”显示黄色三角形及 Tooltip。
- 不显示四行本地会话字段或 Extra Usage。
...
- 无法确认 Claude Code 凭据类型，需要安全降级为本地会话字段。

### 面板内容

Provider 行显示 `Claude Code`，主体区域显示四行独立字段：项目、会话标题、模型、输入/输出 Token。
...
| --- | --- | --- |
| OAuth + API Key | OAuth | Provider、计划、5-Hour、Weekly |
| 仅 OAuth | OAuth | Provider、计划、5-Hour、Weekly |
| 仅 API Key、仅推理 Token 或第三方推理配置 | API 密钥 | Provider、四行本地会话字段 |
| 无法确认凭据类型 | API 密钥 | Provider、四行本地会话字段 |

- Codex 与 Claude Code 均满足上述模式矩阵。
- OAuth 使用窗口和 API 密钥会话字段不会同时出现，切换 Agent 或模式时也不得短暂共存。
- OAuth API 调用失败不会改变已判定的接入模式。

### OAuth 模式

- Codex 显示 Provider、映射后的计划名称、5-Hour 和 Weekly 窗口。
- Claude Code 显示 Provider、映射后的计划名称、5-Hour 和 Weekly 窗口。
- 计划名称显示在 Provider 名称后；原始值为空时只隐藏计划名称，不隐藏 Provider 行。
- 存在当前准确会话的上下文数据时，同时显示 `contextPill`；Usage API 失败不清除该值。
- 每个使用窗口显示进度条、剩余百分比和重置时间，进度条填充表示剩余额度。
- 不显示四行本地会话字段、货币余额、Credits、Extra Usage 或 Rate Limit Reset Credits。
- 429、网络超时或空响应保留当前账户最后一次成功快照，并显示黄色三角形。
- OAuth 凭据失效时通过三角形 Tooltip 提示重新登录，不改变 Halo 状态。
- 不存在成功快照时窗口显示“暂无数据”，Provider 行仍保持可用。
- 正常状态不显示三角形，面板底部不显示状态行。

### API 密钥模式

- Provider 行显示当前 Provider，主体区域显示项目、会话标题、模型和输入/输出 Token 四行独立字段。
- 存在当前准确会话的上下文数据时，同时显示 `contextPill`。
- 会话标题缺失时显示 `--`，不得使用项目名称代替。
- 任一字段缺失不会隐藏其他字段或整个四行字段组。
- `OFFLINE` 时四行均显示 `--`。
- `OFFLINE` 时隐藏 `contextPill`。
- 不显示计划、使用窗口、Usage 状态图标、余额占位或红色服务商错误。
...
- 计划原始值为空时不显示空标签或占位标签，但继续显示 Provider 名称。
