...
- OAuth 模式显示 Provider、计划、5-Hour/Weekly 使用窗口和异常状态图标。
- API 密钥模式显示 Provider、项目、会话标题、模型和输入/输出 Token。
...
9. `contextPill` 与接入模式相互独立；Provider 行始终显示，OAuth 计划名称和异常图标显示在该行。
...
- `AgentHaloMac/DetailsPanel.swift`：增加 Provider/Plan 状态行、异常 Tooltip 和独立会话标题行。
- `AgentHaloCoreChecks/main.swift`：增加认证、映射、缓存和 Coordinator 检查。
- `AgentHaloMac/HaloInteractionChecks.swift`：增加面板互斥、上下文、切换和布局检查。
- 本地化源文件：增加窗口、异常 Tooltip 和会话标题相关键；不修改 Windows 运行时代码。
...
成功快照不包含错误。429、网络错误、认证失败和解析失败只改变 `UsageMonitorState`，不能覆盖最后成功快照。`lastFailure` 只保存可安全映射为 UI 文案的错误分类，不保存服务端响应正文；成功刷新后清空。
...
`contextUsedPercent` 不属于 OAuth 快照；它始终来自当前准确会话的本地数据。`usageWarning` 是经过分类和本地化的安全 Tooltip 文案，不能包含响应正文、凭据或账户标识；API 密钥模式固定为 `nil`。
...
| `pro` | `Pro 20x` |
| 其他非空值 | 按 `_` 分词后转标题格式 |
| 空值 | `nil`，Provider 行不显示计划文本 |

### Claude Code 计划名称

- `subscriptionType` 转为标题格式。
- `rateLimitTier` 中存在 `数字+x` 时追加到名称，例如 `Max 5x`。
- `subscriptionType` 为空时返回 `nil`，Provider 行不显示计划文本。
...
同一时刻只生成一条脱敏 Tooltip 文案。`fresh` 不显示图标；`signInAgain`、过期或失败后的 `stale`、失败后的 `noData` 在 OAuth Provider 行显示一个黄色 `exclamationmark.triangle.fill`，不改变 Halo 状态。新账户或首次刷新尚未完成时，`noData` 没有失败分类，不显示三角形。`DetailsContentResolver` 根据 `status`、`lastFailure`、最后更新时间和冷却时间生成唯一 Tooltip；正常刷新进行中且尚无失败分类时不显示三角形。
...
- 不显示 `OAuth`、`API Key`、“使用情况 / 余额”或“会话详情”等模式或分区标题。
- Provider 行固定在 Agent 切换器下方，在 OAuth、API 密钥和离线状态下均显示。
- Provider 名称使用约 12pt Semibold 主文本；计划名称使用约 11pt Regular 次要文本。
- 面板继续使用现有固定宽度 `268pt`；长文本不得撑宽面板。
- Provider 行在两种模式下占据相同高度；主体高度按内容自动调整。
...
- OAuth 模式在 Provider 名称后显示计划；API 密钥模式只显示 Provider 名称。
- 计划名为空时只隐藏计划文本，不显示 `--`，也不隐藏 Provider 行。
- 计划名过长时单行尾部截断，原生 Tooltip 显示完整名称。
- `fresh` 不显示异常图标；过期或失败后的 `stale`、失败后的 `noData`、`signInAgain` 和 429 冷却状态显示约 10pt 的黄色 `exclamationmark.triangle.fill`。首次刷新尚未完成且没有失败分类时不显示。
- 三角形只提供原生 Tooltip 和可访问性说明，不增加点击行为。
- `stale` Tooltip 提示数据可能已过期并显示最后更新时间；`noData` 显示脱敏失败说明；`signInAgain` 提示重新登录对应 Provider；429 提示请求过于频繁并将在稍后重试。
- 正常后台刷新不显示三角形或加载动画；面板底部不增加数据状态行。
- API 密钥模式不显示 Usage 异常图标。
...
5-Hour    Jul 20    96% Remaining
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Weekly    Jul 17    81% Remaining
━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

- 英文窗口名为 `5-Hour`、`Weekly`；中文为“五小时”、“每周”。
- `UsageWindow.usedPercent` 在展示层转换为 `remainingPercent = 100 - usedPercent`。
- 右侧文案和进度条填充长度都表示剩余额度，不改变现有 AgentHalo 的进度条语义。
- 重置时间显示在窗口名与剩余百分比之间；缺失时隐藏，不显示 `--`。
- 两个窗口行固定保留；单个窗口缺失时该行显示“暂无数据”，进度条为空。
- 重置时间已到但尚未取得新快照时显示“等待刷新”，进度条为空。
- Agent 离线时隐藏会话级 `contextPill`，但继续显示 Provider 行和当前账户最后成功的使用情况快照。
- 不显示四行本地会话字段和本期排除的余额类字段。
...
模型                         gpt-5.4
────────────────────────────────────
输入/输出 Token              输入 12k · 输出 3.2k
```

- 四项使用四个独立 `MetadataRowView`。
- 四行之间沿用当前浅色分隔线；不增加主体标题。
- 会话标题缺失时显示 `--`，不得回退为项目。
- 输入或输出 Token 仅一侧缺失时，只将缺失侧显示为 `--`。
- Token 值沿用当前输入/输出弱强调色。
- 长项目、会话标题和模型单行尾部截断，原生 Tooltip 显示完整内容。
- `OFFLINE` 时四行均显示 `--`，同时隐藏 `contextPill`。
- `OFFLINE` 时仍显示当前 Provider 名称。
- 不显示计划、使用窗口和 OAuth 异常图标。

四行本地会话数据继续来自现有 `SessionSnapshot` 和 Claude 精确会话解析器。Codex 的 `SessionReducer` 需要在 `session_meta` 或后续受支持的会话元数据包含标题时写入现有 `SessionSnapshot.sessionTitle`；上游没有标题字段时保持 `nil`，UI 显示 `--`，不得用项目名称补位。

主体切换时先隐藏两个 Group，再显示目标 Group，避免同一帧短暂共存和面板高度跳变。主体和面板高度不做淡入或尺寸动画，只保留现有 Agent 切换器动画。Agent/Session 切换时立即清除旧的 `contextPill` 和会话字段；旧 Provider 异步结果只能更新自身状态与缓存。
...
- 429、网络、服务端、解析和重新登录错误分类；成功刷新清空 `lastFailure`。
- `DetailsContentResolver` 生成唯一脱敏 Tooltip，正常刷新且无失败分类时不生成警告。
- 重复刷新合并和焦点 Provider 调度。
- `DetailsPanelBody` 主体互斥。

### AgentHaloMac 自检

- Provider 行在 OAuth、API 密钥和离线状态下均显示正确 Provider 名称。
- OAuth 在 Provider 名称后显示计划，API 密钥不显示计划；缺少计划时不显示占位。
- OAuth 显示 5-Hour、Weekly、剩余百分比和表示剩余额度的进度条。
- `fresh` 和首次刷新尚未完成的 `noData` 不显示三角形；过期或失败后的 `stale`、失败后的 `noData`、`signInAgain` 和 429 显示唯一黄色三角形及正确的脱敏 Tooltip。
- 错误 Tooltip 和可访问性文案不包含 Token、响应正文或账户标识。
- 面板底部不显示状态行，正常后台刷新不显示加载动画。
- API 密钥显示四行独立会话字段和浅色分隔线，不显示主体标题或 Usage 异常图标。
- 会话标题不回退为项目。
...
- 被截断的计划、项目、会话标题和模型通过原生 Tooltip 显示完整内容。
...
6. 更新 DetailsPanel：Provider/Plan 状态行、5-Hour/Weekly 剩余额度、异常 Tooltip 和独立会话标题行。
...
- `contextPill`、Provider/计划行、异常图标和主体内容按本设计同时、互斥或隐藏。
