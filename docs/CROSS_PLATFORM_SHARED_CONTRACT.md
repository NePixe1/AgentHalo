...
Codex 的 Plan Mode（计划模式）需要两端在轮次结束时把光环停在「等待用户确认」的
珊瑚色 attention 状态，而不是直接进入绿色 done。判定逻辑只依赖 `~/.codex/sessions`
JSONL 流里的下列字段，两端实现必须保持一致：
...
| `payload.phase == "final_answer"` | `event_msg.agent_message` 与 `response_item.message` payload | 本轮已经给出最终答案。 |

减速器在每轮内维护两个布尔标志：

- `currentTurnIsPlanMode` — 由上述任一 plan 字段置位（取并集，兼容两种事件顺序）。
- `planFinalAnswerSeen` — 当 `currentTurnIsPlanMode` 为真且观测到 `final_answer` 时置位。
...
两端**当前不强制锁蓝**，仅置位 `planFinalAnswerSeen`，视觉上仍按既有
`.thinking` / `.working` 流程显示。如需调整为「最终答案期间锁蓝」，必须两端同步改、
并在 `releaseVersion` 中记录。
